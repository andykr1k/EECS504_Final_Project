...
class Sam3ReIDDataset(Dataset):
    ...
    @Timer(name="Build Dataset", text="Build Dataset: {:.4f} seconds")
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            with Timer("Apply Transforms", text="Apply Transforms: {:.4f} seconds"):
...
class DinoDataLoaderWrapper:
    ...
    """
    dino_harness: dino_lib.OptimizedDinoHarness | None = None
    ...
    @Timer(name="Init DINO", text="Init DINO: {:.4f} seconds")
    def _init_dino(self):
        if self.dino_harness is None:
            self.dino_harness = dino_lib.OptimizedDinoHarness(device=self.device, checkpoint=self.checkpoint, max_side_len=self.max_side_len)
        self.dino_harness.model.eval()

    def __iter__(self):
        for batch in self.dataloader:
            # Convert the images to PIL to prepare to pass to DINO
            with Timer("Convert images for DINO", text=f"Convert {len(batch['images'])} images for DINO:" + " {:.4f} seconds"):
                images = batch["images"]
            ...
            with torch.no_grad():
                ...
                with Timer("Match Segmentations to DINO", text="Match Segmentations to DINO: {:.4f} seconds"):
                    dino_segmentations = self.dino_harness.match_segmentations_to_dino_bool(images, masks)

            with Timer("Process DINO Segmentations", text="Process DINO Segmentations: {:.4f} seconds"):
...
class VideoSlicePKBatchSampler(Sampler):
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            with Timer("Sample PK", text="Sample PK: {:.4f} seconds"):
                batch = []
